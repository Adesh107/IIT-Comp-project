...
export const makeRevision=async(req:Request, res: Response)=>{
    ...
    try{
       ...
       if(user.credits<2){
        return res.status(401).json({message:'add maore credits to make changes'});
       }
...
Return ONLY the enhanced request, nothing else. Keep it concise (1-2 sentences).`
        ...
        const enhancedprompt = promptEnhanceResponce.choices.INDEX[0].message.content;
        await Prisma.Conversation.create({
            data:{
                role:'assistant',
    ...
    Apply the requested changes while maintaining the Tailwind CSS styling approach.`
        ...
        const version = await prisma.version.model.create({
            data: {
...
export const rollbackToVersion =async(req:Request, res: Response)=>{
   ...
   try {
       ...
       if (!project) {
           return res.status(404).json({ messaage: 'Project not found'});
       }

       const version = project.versions.find(()=>version.id === versionId)
...
export const getProjectById =async(req:Request, res: Response)=>{
   ...
   try {
       ...
       if(!projectId || project.isPublished === false || !project?.current_code){
           return res.status(404).json({ message: 'Project not found' });
       }
