...
import { ArrowBigDownDashIcon, EyeIcon, EyeOffIcon, Fullscreen, FullscreenIcon, LaptopIcon, Loader2Icon, MessageSquareIcon, SaveIcon, Sidebar, Smartphone, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react'
...
const Projects = () => {
  ...
  return project ? (
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='max-w-64 sm:max-w'>
        <p className='text-sm text-medium capitalize truncate'>{project.name}</p>
      <p className='text-xs text-gray-400 -mt-0.5'>Previewing last saved version</p></div>
      ...
      <div className='flex-1 flex overflow-auto'>
             <Sidebar isMenuOpen={isMenuOpen} project={project} setProject={(p)=>setProject(p)} isGenerating={isGenerating} setIsGenerating={setIsGenerating}/>
              <div className='flex-1 p-2 pl-0'>
                <ProjectPreview ref={previewRef} project={project} isGenrating={isGenerating} device={device}/>
              </div>
      </div>
   </div> 
  )
